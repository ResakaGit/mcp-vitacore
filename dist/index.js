...
async function main() {
    ...
    if (!GEMINI_API_KEY || GEMINI_API_KEY.trim() === "") {
        failValidation("GEMINI_API_KEY no definida.", "Variable de entorno obligatoria para la API de Gemini.", [
    ...
    if (VITACORE_DB_PATH !== ":memory:") {
        ...
        try {
            ...
            failValidation("No se pudo crear el directorio de la base de datos.", msg, [
    ...
    try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        failValidation("No se pudo inicializar SQLite (better-sqlite3).", msg, [
    ...
    try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        failValidation("Gemini no responde (health check falló).", msg, [
