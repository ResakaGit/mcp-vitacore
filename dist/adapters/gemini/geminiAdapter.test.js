...
describe("GeminiPort (mock)", () => {
    const mockGemini = {
        async generateSessionSummary(steps) {
            if (steps.length === 0)
                return "Sesión sin pasos registrados.";
    ...
    it("generateRefactorPlan returns string", async () => {
        const out = await mockGemini.generateRefactorPlan([{ action: "validación", implications: "repetida" }], "Macro");
        expect(out).toContain("1 pasos");
        expect(out).toContain("presente");
    });
});
//# sourceMappingURL=geminiAdapter.test.js.map