...
export async function resolveArchitecturalParadox(ports: Ports, paradoxId: string): Promise<ToolResult> {
  ...
  try {
    ...
    if (paradox.status === "open" && paradox.analysis) {
      const resolutionSuggestion = await ports.gemini.answerFromContext(
        `Sugiere en 2-3 oraciones cómo resolver esta paradoja arquitectónica: ${paradox.description}. Análisis: ${paradox.analysis}`,
        [{ action: paradox.description, implications: paradox.analysis }]
      );
