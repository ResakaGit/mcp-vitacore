...
describe("submitForBackgroundReview", () => {
  it("returns error when session_id is empty", async () => {
    ...
    const result = await submitForBackgroundReview(ports as never, "");
  ...
  it("returns error when session has no steps", async () => {
    ...
    const result = await submitForBackgroundReview(ports as never, "s1");
    expect(result.isError).toBe(true);
  ...
  it("generates plan and inserts refactor plan", async () => {
    ...
    expect(ports.storage.insertRefactorPlan).toHaveBeenCalledWith(
      expect.objectContaining({ sessionId: "s1", planText: "• Extraer validación" })
    );
  });
});
