...
export function sessionSummaryUser(steps) {
    if (steps.length === 0)
        return "No hay pasos en esta sesión. Responde: Sesión sin pasos registrados.";
    return steps
...
export function evolveMacroUser(currentMacro, recentSessionSummaries) {
...
export function oracleUser(question, contextRecords) {
    const contextBlock = contextRecords.length > 0
        ? contextRecords
...
export function paradoxUser(macro, sessionSummaries) {
...
export function refactorPlanUser(sessionSteps, macro) {
    const stepsBlock = sessionSteps.length > 0
