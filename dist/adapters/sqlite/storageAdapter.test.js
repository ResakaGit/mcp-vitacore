...
describe("storageAdapter", () => {
    describe("initSchema", () => {
        it("creates all tables and macro row", () => {
            ...
            initSchema(db);
            const steps = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name").all();
            expect(steps.map((r) => r.name).sort()).toEqual(["debates", "macro", "paradoxes", "refactor_plans", "sessions", "steps"]);
            const macro = db.prepare("SELECT id, content FROM macro").get();
